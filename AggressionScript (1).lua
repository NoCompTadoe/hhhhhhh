...
menu.hyperlink(asmeme, "get back to work spongebob", "https://www.youtube.com/watch?v=zck8LyxxtfA", "")
...
menu.hyperlink(asmeme, "roblox rp", "https://www.youtube.com/watch?v=Xm3MsViPFSA", "")
...
menu.action(sarcasm, "Execute Script", {}, "Will kill your fps for a short period as it does its thing. Will notify you when it is done. Might throw an error that a label doesn't exist but that is GTA's fault, not mine. THIS OPTION APPLIES MANY JOKES TO YOUR GAME. GO TO YOUR STAND PROFILE.TXT AND DELETE THE BLOCK OF TEXT TO REVERT THEM.", function()
	...
	do_label_preset("TXT_BNTY_NPCB", "~a~ is in your walls. ~a~ is in your walls. ~a~ IS IN YOUR WALLS. ~a~ IS IN YOUR WALLS. ~a~ IS IN YOUR WALLS.")
	do_label_preset("CELL_COOK_N", "Annoying Douche")
	do_label_preset("ALS_TXT_RIV2", "PLEASE RECRUIT ME INTO THE LSPD. I AM NORMAL AND CAN BE TRUSTED WITH WEAPONS.")
	do_label_preset("EF_MONEYANDSERV", "CONTENT HERE")
	do_label_preset("EF_FOODANDDRINK", "CONTENT HERE")
	do_label_preset("EF_MEDIAANDENT", "CONTENT HERE")
	do_label_preset("EF_CLOSESTMATCH", "CONTENT HERE")
	do_label_preset("DESC_THEDIAMONDCASINOANDRESORT_COM", "CONTENT HERE")
	do_label_preset("DESC_FORECLOSURES_MAZE_D_BANK_COM", "CONTENT HERE")
	do_label_preset("DYN8_SPLASH", "Hey can I get uuaghuh uhh ugaaghhh pipe bombs I mean pipe bombs I mean pipe bombs I mean pipe bombs I mean pipe bombs I mean pipe bombs I mean ")
	do_label_preset("DYN8_EXEC_ENTER", "LEAVE SITE")
	do_label_preset("DYN8_EXEC_OFFICE_MAP", "FAT BITCHES IN YOUR AREA")
	do_label_preset("DESC_LIFEINVADER_COM", "Recommended For You: fat bitches fighting over food")
	do_label_preset("EYEDISC_SA", "THROW BRICKS AT HOMELESS PEOPLE")
	menu.trigger_commands("tgame")
	util.toast("Done.")
end)

menu.action(sarcasm, "Exe", {}, "debug option, adds a few more stuffs so I don't waste time running the above option again.", function()

end)

menu.action(asqs, "Слава Украiні", {"ua"}, "glory to ukraine", function(on_click)
	chat.send_message("Слава Украiнi", false, true, true) 
end)

menu.action(asqs, "Taiwan is a country", {"tc"}, "Makes you say the name of this option in simplified chinese in chat. May result in angry replies, download Next Translate by lance to laugh at them", function(on_click)
	chat.send_message("台湾是主权国家", false, true, true) --taiwan is a sovereign country
end)

menu.action(asqs, "China is a province of taiwan", {"ctw"}, "", function(on_click)
	chat.send_message("中国是台湾的一个省", false, true, true) 
end)

menu.action(asqs, "are you guys going trick or treating", {"tot"}, "", function(on_click)
	chat.send_message("are you guys going trick or treating", false, true, true) 
end)

val = 1000
menu.click_slider(asveh, "Vehicle Durability", {""}, "...Does what it says. Increases the explosion resistance of vehicles among some small other things. Will set armor at 100% and add bulletproof tires. 1000 is the default 1 tap to explosions, while 20000 will put any car roughly on par with the nightshark. Explosions are still gonna throw you around.", 500, 150000, 1000, 500, function(val)
	...
	menu.action(ascm, "Send FIB APC", {"fibapc"}, "you heard me.", function(on_click)
		...
		if PATHFIND.GET_CLOSEST_VEHICLE_NODE_WITH_HEADING(offset.x, offset.y, offset.z, outCoords, outHeading, 1, 3.0, 0) then
			...
			for seats = 0, 2 do
				...
				addBlipForEntity(vehicle, 558, 59)
				PED.SET_PED_FIRING_PATTERN(cops, 0xC6EE6B4C)
				VEHICLE.SET_VEHICLE_CUSTOM_PRIMARY_COLOUR(vehicle, 0, 0, 0) --black
				VEHICLE.SET_VEHICLE_MOD(vehicle, 10, 0)
				VEHICLE.SET_VEHICLE_CUSTOM_SECONDARY_COLOUR(vehicle, 0, 0, 0)
				VEHICLE.SET_VEHICLE_MOD_COLOR_1(vehicle, 3, 0, 0) --matte finish
